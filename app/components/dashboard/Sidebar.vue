<template>
  ...
  >
    <div :class="[
      ...
      <!-- Navigation générale -->
      <div class="mb-6">
        <h3 v-if="!isSidebarCollapsed" class="text-xs font-semibold mb-3 transition-colors duration-200 hidden sm:block tracking-wide" style="color: var(--text-primary);">
          Navigation
        </h3>
        
        <nav class="space-y-1" role="navigation" aria-label="Navigation générale">
          ...
          <div class="space-y-1">
            <NuxtLink
              ...
              :style="{
                backgroundColor: $route.path === '/dashboard' ? 'var(--bg-border)' : 'transparent',
            ...
            >
              <Icon 
                ...
                aria-hidden="true"
              />
              <span v-if="!isSidebarCollapsed" class="text-xs sm:text-sm font-medium truncate ml-2">Tableau de bord</span>
            </NuxtLink>
            
            <!-- Sous-menu Vérifications (visible seulement sur la page dashboard avec un projet) -->
            <div v-if="isClient && hasProjects && currentProject?.checklistType && $route.path === '/dashboard' && !isSidebarCollapsed" class="ml-6 space-y-1">
              <!-- Skeleton pendant le chargement du projet -->
              <div v-if="isProjectLoading" class="space-y-2">
                <div v-for="i in 5" :key="`skeleton-category-${i}`" 
          ...
          <NuxtLink
            ...
            :style="{
              backgroundColor: $route.path === '/profile' ? 'var(--bg-border)' : 'transparent',
              color: 'var(--text-primary)'
            }"
          ...
          >
            <Icon 
              ...
              aria-hidden="true"
            />
            <span v-if="!isSidebarCollapsed" class="text-xs sm:text-sm font-medium truncate ml-2">Mon profil</span>
          </NuxtLink>
          ...
          <NuxtLink
            ...
            :style="{
              backgroundColor: $route.path === '/settings' ? 'var(--bg-border)' : 'transparent',
              color: 'var(--text-primary)'
            }"
          ...
          >
            <Icon 
              ...
              aria-hidden="true"
            />
            <span v-if="!isSidebarCollapsed" class="text-xs sm:text-sm font-medium truncate ml-2">Paramètres</span>
          </NuxtLink>
          ...
          <NuxtLink
            ...
            :style="{
              backgroundColor: $route.path === '/help' ? 'var(--bg-border)' : 'transparent',
              color: 'var(--text-primary)' 
            }"
          ...
          >
            <Icon 
              ...
              aria-hidden="true"
            />
            <span v-if="!isSidebarCollapsed" class="text-xs sm:text-sm font-medium truncate ml-2">Aide & Support</span>
          </NuxtLink>
        </nav>
      </div>
      
      <!-- Contenu de la sidebar selon l'état -->
      <div v-if="isClient && hasProjects && currentProject?.checklistType && $route.path === '/dashboard'">
        <!-- Message d'invitation quand aucun projet ou pas de type de checklist -->
        <div v-if="isClient && !isSidebarCollapsed && (!hasProjects || !currentProject?.checklistType) && $route.path === '/dashboard'" class="text-center">
